...
// 2. 데이터베이스에 연결하고 테이블에 Insert 쿼리 실행하기
	...
	try{
		...
		st.setString(3, usernm); 
		
		st.executeUpdate(); // st = con.prepareStatement(sql) 이면 st.executeUpdate()
...
// 3. 특정 페이지로 재요청할 수 있도록 한다(웹브라우저가 sendRedirect("url") 응답받으면 다시 지정된 url로 자동요청)
